using UnityEngine;
using System.Collections;
using UnityEngine.UI;
using UnityEngine.SceneManagement;
using System;
using System.Collections.Generic;
using DG.Tweening;

public class QuizGame : MonoBehaviour
{
    public Button[] answerButtons = new Button[4];
    public Text questionText;

    public int[] answers = new int[4];
    public string[] answersInEnglish = new string[4];
    private int questionsDone = 0;
    int correctAnswerLocation;

    public GameObject[] QuestionPanels;
    public GameObject finalResultPanel;
    public Text resultText;
    private int score = 0;
    private bool gameActive = true;
    public NumbersVoice numbersVoice;
    public Image star1;
    public Image star2;
    public Image star3;
    public Sprite filledStar;
    public Sprite emptyStar;

    public Text scoreText;
    public Button questionButton;
    public Sprite one, two, three, four, five, six, seven, eight, nine, ten;
    public Sprite correct, wrong;
    public GameObject feedbackButton;

    private bool hasShownTutorial = false;
    private int totalQuestions = 50;
    private bool isFinalResult = false;

    void Start()
    {
        score = 0;
        questionsDone = 0;
        UpdateScoreText();
        newQuestion();
    }

    void newQuestion()
    {
        int numberAsked = UnityEngine.Random.Range(1, 11);

        switch (numberAsked)
        {
            case 1: questionButton.GetComponent<Image>().overrideSprite = one; break;
            case 2: questionButton.GetComponent<Image>().overrideSprite = two; break;
            case 3: questionButton.GetComponent<Image>().overrideSprite = three; break;
            case 4: questionButton.GetComponent<Image>().overrideSprite = four; break;
            case 5: questionButton.GetComponent<Image>().overrideSprite = five; break;
            case 6: questionButton.GetComponent<Image>().overrideSprite = six; break;
            case 7: questionButton.GetComponent<Image>().overrideSprite = seven; break;
            case 8: questionButton.GetComponent<Image>().overrideSprite = eight; break;
            case 9: questionButton.GetComponent<Image>().overrideSprite = nine; break;
            case 10: questionButton.GetComponent<Image>().overrideSprite = ten; break;
        }

        correctAnswerLocation = UnityEngine.Random.Range(0, 4);

        for (int i = 0; i < answers.Length; i++)
        {
            if (i == correctAnswerLocation)
            {
                answers[i] = numberAsked;
            }
            else
            {
                int wrongAnswer = UnityEngine.Random.Range(1, 11);
                while (wrongAnswer == numberAsked || Array.IndexOf(answers, wrongAnswer) > -1)
                {
                    wrongAnswer = UnityEngine.Random.Range(1, 11);
                }
                answers[i] = wrongAnswer;
            }
        }

        for (int i = 0; i < answers.Length; i++)
        {
            switch (answers[i])
            {
                case 1: answersInEnglish[i] = "ONE"; break;
                case 2: answersInEnglish[i] = "TWO"; break;
                case 3: answersInEnglish[i] = "THREE"; break;
                case 4: answersInEnglish[i] = "FOUR"; break;
                case 5: answersInEnglish[i] = "FIVE"; break;
                case 6: answersInEnglish[i] = "SIX"; break;
                case 7: answersInEnglish[i] = "SEVEN"; break;
                case 8: answersInEnglish[i] = "EIGHT"; break;
                case 9: answersInEnglish[i] = "NINE"; break;
                case 10: answersInEnglish[i] = "TEN"; break;
            }
        }

        questionText.text = $"Question {questionsDone + 1} of {totalQuestions}";
        for (int i = 0; i < answerButtons.Length; i++)
        {
            answerButtons[i].GetComponentInChildren<Text>().text = answersInEnglish[i];
        }

        if (!hasShownTutorial && Tutorial.instance != null)
        {
            GameObject correctAnswerObject = answerButtons[correctAnswerLocation].gameObject;
            Tutorial.instance.StartTutorialAtAnswer(correctAnswerObject);
            hasShownTutorial = true;
        }
    }

    public void checkAnswer(int buttonNumber)
    {
        if (!gameActive) return;

        if (buttonNumber == correctAnswerLocation)
        {
            score++;
            UpdateScoreText();
            answerButtons[buttonNumber].GetComponent<Image>().color = Color.green;
            //numbersVoice.PlayCorrectNumberSound(answers[buttonNumber]);
            feedbackButton.GetComponent<Image>().overrideSprite = correct;
            feedbackButton.GetComponent<Image>().color = new Color32(33, 171, 0, 255);
            StartCoroutine("ContinueAfterFeedback");
        }
        else
        {
            answerButtons[buttonNumber].GetComponent<Image>().color = Color.red;
            numbersVoice.PlayWrongSound();
            feedbackButton.SetActive(true);
            feedbackButton.GetComponent<Image>().overrideSprite = wrong;
            feedbackButton.GetComponent<Image>().color = Color.red;
        }
    }

    void UpdateScoreText()
    {
        scoreText.text = $"Score: {score}/{totalQuestions}";
    }

    IEnumerator ContinueAfterFeedback()
    {
        gameActive = false;
        feedbackButton.SetActive(true);
        yield return new WaitForSeconds(1.5f);

        for (int i = 0; i < 4; i++)
        {
            answerButtons[i].GetComponent<Image>().color = new Color32(254, 238, 220, 255);
        }

        feedbackButton.SetActive(false);
        questionsDone++;

        if (questionsDone % 5 == 0 && questionsDone < totalQuestions)
        {
            ShowSegmentResult();
            yield break;
        }

        if (questionsDone < totalQuestions)
        {
            newQuestion();
        }
        else
        {
            ShowFinalResult();
        }

        gameActive = true;
    }

    void ShowSegmentResult()
    {
        isFinalResult = false;
        finalResultPanel.SetActive(true);
        resultText.text = $"Progress: {score}/{questionsDone}";

        if (Initialize.Instance != null)
        {
            Initialize.Instance.LoadInterstitialAd();
            Initialize.Instance.ShowInterstitialAd();
        }
    }

    void ShowFinalResult()
    {
        isFinalResult = true;
        finalResultPanel.SetActive(true);
        resultText.text = $"Final Score: {score}/{totalQuestions}";

        int percentage = Mathf.RoundToInt(((float)score / totalQuestions) * 100);

        if (percentage >= 80)
        {
            star1.sprite = filledStar;
            star2.sprite = filledStar;
            star3.sprite = filledStar;
            //numbersVoice.PlayLevelCompleteSound();
        }
        else if (percentage >= 50)
        {
            star1.sprite = filledStar;
            star2.sprite = filledStar;
            star3.sprite = emptyStar;
            //numbersVoice.PlayLevelCompleteSound();
        }
        else if (percentage >= 20)
        {
            star1.sprite = filledStar;
            star2.sprite = emptyStar;
            star3.sprite = emptyStar;
        }
        else
        {
            star1.sprite = emptyStar;
            star2.sprite = emptyStar;
            star3.sprite = emptyStar;
        }

        PlayerPrefs.SetInt("Level2", 1);
        PlayerPrefs.Save();
    }

    public void ContinueGameAfterSegment()
    {
        finalResultPanel.SetActive(false);

        if (isFinalResult)
        {
            return; // Final result stays until player presses restart or quit
        }

        newQuestion();
        gameActive = true;
    }

    public void restartLevel()
    {
        SceneManager.LoadScene("NumbersGameLevel1");
    }

    public void QuitGame()
    {
        SceneManager.LoadScene("NumbersLearning");
    }
}
